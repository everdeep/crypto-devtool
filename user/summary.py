...
from cryptolib.model import UserModel, PortfolioModel
...
class UserSummary:
            ...
            while True:
                ...
                if menu_entry_index == 0:
                    ...
                    while not Confirm.ask(
                        f"Is the following email correct? {email}", default=True
                    ):
                        email = Prompt.ask("Please enter the email of the user")
    ...
    def display_user_summary(self, user):
        ...
        console.print(
            f"Name: {user.first_name} {user.last_name}", style="italic magenta"
        )
    ...
    def display_bot_summary(self, user):
        ...
        for bot in user.currency_pair_configs:
            ...
            console.print(
                f"Initial balance: {bot.allocated_balance}", style="italic magenta"
            )
            console.print(
                f"Currency: {bot.currency_free + bot.currency_locked}",
                style="italic magenta",
            )
            console.print(
                f"Asset: {bot.asset_free + bot.asset_locked}", style="italic magenta"
            )
    ...
    def display_bot_config_summary(self, user):
        ...
        for bot in user.currency_pair_configs:
            if bot.currency_pair != options[sub_menu_index]:
                continue
            console.print(
                f"Bot config for user: {user.email} -> {bot.currency_pair}",
                style="bold magenta",
            )
            console.print("====================================", style="bold magenta")
            console.print(f"ID: {bot.id}", style="italic magenta")
            ...
            for config in bot.strategy_config:
                console.print(
                    f"--> {config.key}: {config.value}", style="italic magenta"
                )
            print()
    ...
    def display_order_summary(self, user):
        ...
        if sub_menu_index == 0:
            ...
            for order in user.orders:
                ...
                console.print(
                    f"Currency pair: {order.currency_pair}", style="italic magenta"
                )
                ...
                console.print(
                    f"Limit price: {order.limit_price}", style="italic magenta"
                )
                console.print(
                    f"Is autotraded: {order.is_autotraded}", style="italic magenta"
                )

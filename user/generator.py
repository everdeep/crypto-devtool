...
class UserGenerator:
        ...
        while not Confirm.ask("Are you happy with these settings?", default=True):
            ...
            if menu_entry_index == 0:
                ...
                self.password_plain = Prompt.ask(
                    "What is the password of the user?",
                    default=generate_random_string(10),
                )
                self.password = bcrypt.hashpw(
                    self.password_plain.encode("utf-8"), bcrypt.gensalt()
                )
    ...
    def set_bot(self):
        is_setup_bot = Confirm.ask(
            "Do you want to setup a bot for this user?", default=True
        )
        if is_setup_bot:
    ...
    def display_overview(self):
        ...
        console.print(
            f"Starting balance: {self.starting_balance}", style="italic green"
        )

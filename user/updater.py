...
from cryptolib.utils import generate_random_string
...
class UserUpdater:
    def update(self):
        ...
        while not Confirm.ask(
            f"Are you sure you want to update {email}?", default=True
        ):
            email = Prompt.ask(
                "What is the email of the user you would like to update?"
            )
        ...
        while True:
            ...
            if menu_entry_index == 0:
                ...
                if result is None:
                    ...
                    console.print(
                        f"Email {new_email} already exists!", style="bold red"
                    )
            elif menu_entry_index == 1:
                new_password = Prompt.ask(
                    "What is the new password of the user?",
                ...
                user.is_admin = Confirm.ask(
                    "Do you want to make this user an admin?", default=False
                )
                session.commit()
                console.print(
                    f"Admin status updated successfully!\n", style="bold green"
                )
    ...
    def display_portfolio_summary(self, portfolio, user_id, email):
        ...
        for balance in portfolio.balances:
            console.print(
                f"Asset: {balance.asset} -> ${balance.total}", style="italic magenta"
            )
        print()
    ...
    def update_balance(self, session, user):
        ...
        while True:
            ...
            if menu_entry_index == 0:
                ...
                console.print(
                    "Warning: This will reset the total balance of the asset!",
                    style="bold yellow",
                )
                console.print(
                    "Any currently locked balances will remain due to order updates.",
                    style="bold yellow",
                )
                ...
                total = FloatPrompt.ask(
                    "How much balance do you want to set?", default=0.0
                )
    ...
    def update_api_key(self, session, user):
        ...
        use_default_key = Confirm.ask(
            "Do you want to use the default api key?", default=True
        )
    ...
    def update_bot(self, session, user):
        ...
        while True:
            ...
            if menu_entry_index == 0:
                console.print(
                    "Warning: This will not automatically transfer funds for you!",
                    style="bold yellow",
                )
                console.print(
                ...
                session.commit()
                console.print(
                    f"Currency pair updated successfully!\n", style="bold green"
                )
            elif menu_entry_index == 1:
                ...
                new_strategy_config = BotGenerator().set_strategy_config(
                    bot.strategy, bot.strategy_config
                )
                ...
                session.commit()
                console.print(
                    f"Strategy config updated successfully!\n", style="bold green"
                )
            elif menu_entry_index == 4:
                console.print("What would you like to do?")
                ...
                session.commit()
                console.print(
                    f"Bot active status updated successfully!\n", style="bold green"
                )

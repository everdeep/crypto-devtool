...
from cryptolib.utils import generate_random_string, generate_random_digit
from cryptolib.enums import Interval, StrategyParams, StrategyType, ExchangeType
...
class BotGenerator:
    ...
    def generate(self) -> dict:
        self.name = Prompt.ask(
            "What is the name of the bot?", default=f"Bot#{generate_random_digit(4)}"
        )
        ...
        while not Confirm.ask("Are you happy with your bot settings?", default=True):
            ...
            if menu_entry_index == 0:
                self.name = Prompt.ask(
                    ...
                    default=f"Bot#{generate_random_digit(4)}",
                )
                ...
                self.symbol = Prompt.ask(
                    "What is the target symbol?", default="BTCUSDT"
                )
            elif menu_entry_index == 6:
                self.is_active = Confirm.ask(
                    "Do you want to activate this bot?", default=False
                )
    ...
    def set_exchange(self, current_exchange=None):
        if current_exchange:
            console.print(
                f"Current exchange: {current_exchange.value}", style="bold yellow"
            )

        console.print("Please select the exchange to use?")
        options = [ExchangeType.BINANCE.value, "Cancel"]
        terminal_menu = TerminalMenu(options)
        menu_entry_index = terminal_menu.show()
    ...
    def set_strategy(self, current_strategy=None):
        ...
        if current_strategy:
            console.print(
                f"Current strategy: {current_strategy.value}", style="bold yellow"
            )
    ...
    def set_strategy_config(self, strategy=None, current_strategy_config=None):
        ...
        for key, value in StrategyParams[self.strategy.name].value.items():
            strategy_config[key] = IntPrompt.ask(
                f"What is the {key} for {self.strategy.value}?",
                default=random.randint(value[0], value[1]),
            )
